...
|---|---------|--------------------|------|--------|
| ☐ | Control Center shell + utility actions | `src/dao/browser/ui/views/dao_control_center_*`, `dao_native_share_mac.*`, `widget_delegate.h.patch` (QR dialog friend) | 🟢 | Popup opens anchored to sidebar/control button, outside click closes it, Share/QR/Lock/More actions render and execute; More menu clears only the current site's cache/cookies |
