...
- **DaoAddressBarView** (`dao_address_bar_view.{h,cc}`) — Embedded URL pill (14px radius), referenced by sidebar / command bar / control center; adapts text color to background luminance
  ...
  - The stop/reload button and page-load glow follow Chromium's loading UI
    state. Late iframe loads and same-document navigation stay quiet, including
    when switching tabs; normal page navigation still shows loading feedback.
...
- **Scoped semantic page automation** (`automation/dao_page_tools.{h,cc}`) — Agent and MCP clients can query a bounded set of elements by selector/ref scope, role, accessible name, text, visibility, enabled state, and first/last/index selection. Ref scopes require the document and snapshot tokens that produced the ref, preventing a reused ref label from resolving inside a newer snapshot. Query and full-tree results carry document and snapshot tokens; `click_by_ref` revalidates those tokens plus optional URL, visibility, enabled, text, role, and ancestor preconditions in the same page script that dispatches the click, so navigation, a newer snapshot, or a changed target fails closed.
- **Batched Agent browser actions** (`resources/agent/pi_tool_adapter.ts`, `agent_bridge.ts`) — Browser tools emitted in the same model response execute sequentially through the existing runtime. A failed or invalid earlier tool call skips later browser actions in that response, including legacy error results without a code. The default prompt batches only known arguments and observes again at decision boundaries; callers must reacquire refs after any successful query or tree snapshot.
- **Guarded form input and semantic waits** (`automation/dao_page_tools.{h,cc}`) — Shared Agent/MCP `fill_by_ref` replaces or clears a visible, enabled, writable text input or textarea after validating document/snapshot/ref tokens, optional click preconditions, and focus. A synthetic, cancelable `beforeinput` hook runs in the same page task as the Chromium editor operation; cancellation preserves the value, and focus, target identity, editability, and preconditions are checked again after handlers run. The editor restores the full selection before replacing or clearing and emits trusted `input` events for controlled fields; success describes the local value, not submission or task completion. `press_key_chord` dispatches trusted Chromium key events using US-layout key names. Keydown and keyup are queued together so cancelling or timing out while a page handles keydown does not leave the key held. Shortcut letters are case-insensitive without adding Shift; explicit Shift and standalone uppercase letters retain their meaning. Supported macOS editor commands include Cmd+A, Cmd+ArrowLeft/Right for line boundaries, Cmd+ArrowUp/Down for document boundaries, Shift selection variants, Cmd+Backspace, and Cmd+Z/Shift+Z, alongside page defaults such as Enter and Tab. `wait_for_element` reuses semantic query filters with bounded 100 ms polling, a 30-second maximum, and cancellation/navigation cleanup; pending waits preserve refs, success creates a fresh snapshot, and exact zero matches requires an existing scope. Agent bridge deadlines accommodate the full native element/network wait budgets.
- **Session-scoped DevTools automation** (`automation/dao_devtools_tools.{h,cc}`) — Agent and MCP clients share native network, console, and page-resource tools. Network tracking returns a monotonic cursor, and `wait_for_network_response` waits for a later completed response by wildcard URL, method, status, or a bounded simple dotted JSON path while projecting only requested values. Domain requests persist independently from confirmed state; events arriving while an enable attempt is pending are staged for that exact generation and host, then committed by a matching success or dropped after all matching attempts fail, cancellation, or a binding change. Clear actions remove both committed and currently staged events without cancelling the matching enable attempt, so only later events can commit. Within one binding, confirmation is monotonic after any successful enable and resets only when the binding changes. Network and console capture retain recent entries under UTF-8-safe field, entry-count, and aggregate-byte budgets with explicit dropped/truncated metadata. One iterative, deduplicating resource traversal synthesizes each frame's Document and reports item, URL-byte, depth, source, scan, and match limits separately. Resource reads require the current exact `(frame, URL)` tree entry when a frame is supplied and reject known decoded sizes over 1 MiB before fetching content; independent per-command response ceilings cover missing metadata. Resource search reports attempted, successfully searched, failed, and content-limited sources plus explicit incompleteness, including bounded Document DOM ingress.
...
- **Versioned NDJSON protocol** (`mcp/dao_mcp_protocol.{h,cc}`) — Protocol version 1 supports `hello`, `tools/list`, `tools/call`, and `tools/cancel`, with an 8 MiB line ceiling, 64-request/8 MiB pending-ingress budget, and structured errors. Catalog discovery is available before approval, while execution waits for approval and the external automation lease. Every MCP tool advertises a `reason` argument required on the first call of each connection; missing, blank, invalid UTF-8, or reasons over 1024 UTF-8 bytes fail before opening a prompt. Later calls may omit it, and this permission metadata is removed before tool execution.
