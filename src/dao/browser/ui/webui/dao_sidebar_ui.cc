...
class DeleteFolderDialog : public views::DialogDelegate {
 public:
  explicit DeleteFolderDialog(base::OnceClosure accept_callback) {
    SetOwnedByWidget(OwnedByWidgetPassKey());
    SetModalType(ui::mojom::ModalType::kWindow);
    SetShowCloseButton(false);
    SetTitle(l10n_util::GetStringUTF16(IDS_DAO_DELETE_FOLDER_DIALOG_TITLE));
    SetButtonLabel(
        ui::mojom::DialogButton::kOk,
        l10n_util::GetStringUTF16(IDS_DAO_DELETE_FOLDER_DIALOG_CONFIRM));
    SetButtonLabel(
        ui::mojom::DialogButton::kCancel,
        l10n_util::GetStringUTF16(IDS_DAO_DELETE_FOLDER_DIALOG_CANCEL));
    SetButtonStyle(ui::mojom::DialogButton::kOk,
                   ui::ButtonStyle::kProminent);
    SetAcceptCallback(std::move(accept_callback));

    auto description = std::make_unique<views::Label>(l10n_util::GetStringUTF16(
  ...
  DeleteFolderDialog(const DeleteFolderDialog&) = delete;
  DeleteFolderDialog& operator=(const DeleteFolderDialog&) = delete;

  ~DeleteFolderDialog() override = default;
};
...
void DaoSidebarUIHandler::SetBrowser(Browser* browser) {
  ...
  if (browser_changed) {
    ...
    pinned_items_profile_refresh_pending_ = false;
  ...
  if (browser_) {
    ...
    if (browser_changed) {
      RegisterPinnedItemsProfileHandler();
    }
...
// static
void DaoSidebarUIHandler::WaitForFolderFileTasksForTesting() {
...
bool DaoSidebarUIHandler::LoadPinnedItemsForTesting(const std::string& json) {
  ...
  if (!pinned_tab_model_.LoadFromJson(json)) {
    return false;
  }
  PublishPinnedItemsToProfileHandlers();
  return true;
}
...
views::Widget* DaoSidebarUIHandler::ShowDeleteFolderDialogForTesting(
    const std::string& folder_id) {
  return ShowDeleteFolderDialog(folder_id);
}
...
void DaoSidebarUIHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&DaoSidebarUIHandler::HandleShowDeleteFolderDialog,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "showTabTooltip",
...
base::ListValue DaoSidebarUIHandler::BuildPinnedItems() {
  ...
  for (const DaoPinnedTabItem& item : pinned_tab_model_.items()) {
    const auto [open_browser, open_tab_index] =
        FindOpenPinnedTabForItem(item);
    TabStripModel* open_model =
        open_browser ? open_browser->tab_strip_model() : nullptr;
    content::WebContents* contents =
        open_model ? open_model->GetWebContentsAt(open_tab_index) : nullptr;
    ...
    dict.Set("isOpen", open_browser != nullptr);
    dict.Set("openTabIndex",
             open_browser == browser_ ? open_tab_index : -1);
    dict.Set("openTabId", contents ? GetSidebarTabId(contents) : std::string());
    dict.Set("isActive",
             open_model && open_tab_index == open_model->active_index());
...
bool DaoSidebarUIHandler::ReconcilePinnedItems(bool restore_complete) {
  ...
  auto append_runtime_tabs =
      [&runtime_tabs](Browser* candidate_browser) {
        if (!candidate_browser) {
          return;
        }
        TabStripModel* candidate_model =
            candidate_browser->tab_strip_model();
        for (int i = 0; i < candidate_model->count(); ++i) {
          if (!candidate_model->IsTabPinned(i)) {
            continue;
          }
          content::WebContents* contents =
              candidate_model->GetWebContentsAt(i);
          if (contents) {
            runtime_tabs.push_back(
                {i, contents, GetSidebarTabId(contents)});
          }
        }
      };
  append_runtime_tabs(browser_);
  if (ProfileBrowserCollection* collection =
          ProfileBrowserCollection::GetForProfile(browser_->profile())) {
    collection->ForEach(
        [this, &append_runtime_tabs](
            BrowserWindowInterface* browser_window) {
          Browser* candidate =
              browser_window->GetBrowserForMigrationOnly();
          if (candidate != browser_) {
            append_runtime_tabs(candidate);
          }
          return true;
        });
  }
...
std::pair<Browser*, int> DaoSidebarUIHandler::FindOpenPinnedTabForItem(
    const DaoPinnedTabItem& item) const {
  if (!browser_ || item.backing_tab_id.empty()) {
    return {nullptr, -1};
  }

  auto find_in_browser =
      [&item](Browser* candidate_browser) {
        if (!candidate_browser) {
          return -1;
        }
        TabStripModel* candidate_model =
        ...
        for (int i = 0; i < candidate_model->count(); ++i) {
          ...
          content::WebContents* contents =
              candidate_model->GetWebContentsAt(i);
          if (contents &&
              GetSidebarTabId(contents) == item.backing_tab_id) {
            return i;
          }
        }
        return -1;
  ...
  int index = find_in_browser(browser_);
  if (index >= 0) {
    return {browser_, index};
  }
  ...
  if (ProfileBrowserCollection* collection =
          ProfileBrowserCollection::GetForProfile(browser_->profile())) {
    collection->ForEach(
        [this, &find_in_browser, &found_browser, &found_index](
            BrowserWindowInterface* browser_window) {
          Browser* candidate =
              browser_window->GetBrowserForMigrationOnly();
          if (candidate == browser_) {
          ...
          const int candidate_index = find_in_browser(candidate);
  ...
  return {found_browser, found_index};
}

void DaoSidebarUIHandler::RegisterPinnedItemsProfileHandler() {
  ...
  state.handlers.insert(this);
  AdoptPinnedItemsFromProfileState();
}

void DaoSidebarUIHandler::UnregisterPinnedItemsProfileHandler() {
  Profile* profile = browser_ ? browser_->profile() : nullptr;
  if (!profile) {
    return;
  }

  auto& states = GetPinnedItemsProfileStates();
  auto state_it = states.find(profile);
...
bool DaoSidebarUIHandler::AdoptPinnedItemsFromProfileState() {
  Profile* profile = browser_ ? browser_->profile() : nullptr;
  if (!profile) {
    return false;
  }
  ...
  auto& states = GetPinnedItemsProfileStates();
  auto state_it = states.find(profile);
  if (state_it == states.end() || !state_it->second.initialized) {
    return false;
  }
  if (!pinned_tab_model_.LoadFromJson(state_it->second.json)) {
    LOG(ERROR) << "Failed to adopt the in-memory Dao pinned tab state";
    return false;
  }
  pinned_items_loaded_ = true;
  pinned_items_auto_save_enabled_ = true;
...
void DaoSidebarUIHandler::PublishPinnedItemsToProfileHandlers() {
  Profile* profile = browser_ ? browser_->profile() : nullptr;
  if (!profile) {
    return;
  }
  ...
  state.json = pinned_tab_model_.ToJson();
  for (DaoSidebarUIHandler* handler : state.handlers) {
    if (handler == this || !handler->browser_ ||
        handler->browser_->profile() != profile) {
      continue;
    }
    if (!handler->pinned_tab_model_.LoadFromJson(state.json)) {
      LOG(ERROR) << "Failed to synchronize the in-memory Dao pinned tab state";
      continue;
    }
    handler->pinned_items_loaded_ = true;
    handler->pinned_items_auto_save_enabled_ = true;
    handler->SchedulePinnedItemsProfileRefresh();
  }
}

void DaoSidebarUIHandler::SchedulePinnedItemsProfileRefresh() {
  if (pinned_items_profile_refresh_pending_) {
    return;
  }
  pinned_items_profile_refresh_pending_ = true;
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
      FROM_HERE,
      base::BindOnce(
          [](base::WeakPtr<DaoSidebarUIHandler> self) {
            ...
            self->pinned_items_profile_refresh_pending_ = false;
  ...
  GetPinnedTabsFileTaskRunner()->PostTaskAndReplyWithResult(
      ...
      base::BindOnce(
          [](base::WeakPtr<DaoSidebarUIHandler> self,
            ...
            if (self->AdoptPinnedItemsFromProfileState()) {
              if (self->IsJavascriptAllowed()) {
                self->PushFullState();
              }
              return;
            ...
            if (result.success &&
                self->pinned_items_auto_save_enabled_) {
              self->PublishPinnedItemsToProfileHandlers();
            }
            if (self->IsJavascriptAllowed()) {
              self->PushFullState();
            }
...
void DaoSidebarUIHandler::UnpinPinnedItemById(std::string id,
                                              int target_index) {
  if (!browser_) {
    return;
  }
  ...
  const auto [open_browser, open_tab_index] =
      FindOpenPinnedTabForItem(*item);
  if (open_browser) {
    TabStripModel* model = open_browser->tab_strip_model();
    ...
    if (target_index >= 0 && contents) {
      if (open_browser != browser_) {
        const int source_session_id = open_browser->session_id().id();
        const int source_index = model->GetIndexOfWebContents(contents);
        if (source_index == TabStripModel::kNoTab ||
            !ExecuteCrossWindowTabMove(browser_, source_session_id,
                                       source_index, target_index)) {
          const int current_index = model->GetIndexOfWebContents(contents);
          if (current_index != TabStripModel::kNoTab) {
            model->SetTabPinned(current_index, true);
          }
          return;
        }
      } else {
...
void DaoSidebarUIHandler::ActivateOrOpenPinnedItem(const std::string& id) {
  ...
  const auto [open_browser, open_tab_index] =
      FindOpenPinnedTabForItem(*item);
  if (open_browser) {
    open_browser->tab_strip_model()->ActivateTabAt(open_tab_index);
    if (open_browser->window()) {
      open_browser->window()->Activate();
    }
...
void DaoSidebarUIHandler::ClosePinnedItemTab(std::string id) {
  if (!browser_) {
    return;
  }
  ...
  open_browser->tab_strip_model()->CloseWebContentsAt(
      open_tab_index, TabCloseTypes::CLOSE_USER_GESTURE);
  DaoPinnedTabItem* closed_item = pinned_tab_model_.FindById(id);
  if (closed_item &&
      pinned_tab_model_.SetState(id, DaoPinnedTabState::kDormant)) {
    SavePinnedItems();
  }
  if (IsJavascriptAllowed()) {
    PushFullState();
...
void DaoSidebarUIHandler::OnTabDragEnded(std::string tab_id,
                                         const gfx::Point& screen_point) {
  if (!browser_ || browser_->window()->GetBounds().Contains(screen_point)) {
    return;
  }
  DetachTabToNewWindow(browser_, tab_id, screen_point);
}
...
void DaoSidebarUIHandler::HandleMoveTabCrossWindow(
  ...
  ExecuteCrossWindowTabMove(
      browser_, args[0].GetIfInt().value_or(-1),
      args[1].GetIfInt().value_or(-1), args[2].GetIfInt().value_or(-1),
      args.size() > 3 && args[3].is_string() ? args[3].GetString()
                                             : std::string());
}

void DaoSidebarUIHandler::HandleDetachTabToNewWindow(
    const base::ListValue& args) {
  if (!browser_ || args.size() < 3 || !args[0].is_int() || !args[1].is_int() ||
      !args[2].is_int()) {
    return;
  }
  const int index = ResolveDraggedTabIndex(browser_, args[0].GetInt(), {});
  if (index < 0) {
    return;
  }
  DetachTabToNewWindow(
      browser_,
      GetOrCreateSidebarTabId(
          browser_->tab_strip_model()->GetWebContentsAt(index)),
      gfx::Point(args[1].GetInt(), args[2].GetInt()));
}
...
void DaoSidebarUIHandler::HandleShowTabContextMenu(
  ...
  context_menu_tab_id_ = GetOrCreateSidebarTabId(contents);
...
void DaoSidebarUIHandler::HandlePinTab(const base::ListValue& args) {
  ...
  const int index = ResolveDraggedTabIndex(
      browser_, args[0].GetIfInt().value_or(-1),
      args.size() > 2 && args[2].is_string() ? args[2].GetString()
                                             : std::string());
  PinTabAtIndex(index, args.size() > 1 ? args[1].GetIfInt().value_or(-1) : -1);
}
...
void DaoSidebarUIHandler::HandleShowPinnedItemContextMenu(
  ...
  int screen_x = args[1].GetIfInt().value_or(0);
  int screen_y = args[2].GetIfInt().value_or(0);
  bool is_open = FindOpenPinnedTabForItem(*item).first != nullptr;
...
void DaoSidebarUIHandler::HandleShowFolderContextMenu(
    const base::ListValue& args) {
  if (!browser_ || args.size() < 3 || !args[0].is_string() ||
  ...
  const int screen_x = args[1].GetInt();
  const int screen_y = args[2].GetInt();
  ...
  tab_context_menu_model_->AddItem(
      kFolderUnfolder,
      l10n_util::GetStringUTF16(IDS_DAO_FOLDER_CONTEXT_UNFOLDER));
  tab_context_menu_model_->AddItem(
      kFolderDelete,
...
void DaoSidebarUIHandler::HandleShowDeleteFolderDialog(
    const base::ListValue& args) {
  ...
  ShowDeleteFolderDialog(folder_id);
}
...
views::Widget* DaoSidebarUIHandler::ShowDeleteFolderDialog(
    const std::string& folder_id) {
  if (!browser_ || folder_id.empty()) {
    return nullptr;
  }

  auto dialog = std::make_unique<DeleteFolderDialog>(base::BindOnce(
      &DaoSidebarUIHandler::OnDeleteFolderDialogAccepted,
...
void DaoSidebarUIHandler::OnDeleteFolderDialogAccepted(
...
bool DaoSidebarUIHandler::IsCommandIdEnabled(int command_id) const {
  ...
  if (command_id == kMoveTabToNewWindow) {
    ...
    return index >= 0 && chrome::CanMoveTabsToNewWindow(browser_, {index});
  }
  ...
  if (command_id == kPinnedOpen || command_id == kPinnedUnpin ||
    ...
    switch (command_id) {
      case kPinnedOpen:
        return FindOpenPinnedTabForItem(*item).first == nullptr &&
...
void DaoSidebarUIHandler::ExecuteCommand(int command_id, int event_flags) {
  ...
  if (command_id == kMoveTabToNewWindow) {
    const int index =
        ResolveDraggedTabIndex(browser_, -1, context_menu_tab_id_);
    if (index >= 0 && chrome::CanMoveTabsToNewWindow(browser_, {index})) {
      chrome::MoveTabsToNewWindow(browser_, {index});
    }
  ...
  if (command_id == kFolderRename || command_id == kFolderUnfolder ||
    ...
    if (IsJavascriptAllowed() && !context_menu_folder_id_.empty()) {
      const char* command = command_id == kFolderRename ? "rename" :
          command_id == kFolderUnfolder ? "unfolder" : "delete";
      FireWebUIListener(
