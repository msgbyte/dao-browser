...
- **Never run bare `npm run export`** — always scope it with `npm run export -- <file>` to export exactly the file(s) you edited. Bare export has two destructive side effects:
  1. **Falsely creates patches for branding-managed files.** `npm run import` copies assets from `branding/` into `engine/src/chrome/app/theme/chromium/*` (logos, BRANDING manifest, app.icns), `ui/webui/resources/images/chrome_logo_dark.svg`, and dynamically rewrites the version string in `chrome/browser/ui/webui/version/version_ui.cc`. These are intentionally NOT in the patch system (see `scripts/commands/import.ts` brandingMap). Bare export sees them as "modified in engine/" and writes new patches under `src/patches/chrome/app/theme/`, `src/patches/ui/webui/resources/images/`, `src/patches/chrome/browser/ui/webui/version/`. These "patches" must be deleted — committing them would freeze the assets and break the dynamic branding pipeline.
  2. **Rewrites every existing patch with stripped trailing whitespace.** Export recomputes blob hashes for all ~190 patches and re-emits their diffs, which strips trailing whitespace from context lines. Some of those context lines have semantically significant trailing whitespace; the resulting patch will fail to apply later with "corrupt patch at line N". If you must do a bulk export, run `git restore src/patches/` afterward on every patch you did not intentionally edit.
